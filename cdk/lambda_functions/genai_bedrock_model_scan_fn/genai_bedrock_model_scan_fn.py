    ...
    if is_websocket_event and connection_id:
        ...
        try:
            ...
            if connection_state != "OPEN":
                logger.info(
                    f"WebSocket connection is not open (state: {connection_state})"
                )
                return
        except ClientError:
            logger.warn(
        ...
        try:
            allowed, not_allowed_message = commons.validate_jwt_token(
                cognito_client, user_cache, allowlist_domain, access_token
            )
        except ClientError as e:
            allowed, not_allowed_message = (
                (False, "Your Access Token has expired. Please log in again.")
    ...
    try:
        if "imported-model" in model_id and prompt_type == "TEXT":
            ...
            if prompt_type == "IMAGE":
                ...
                doc_bytes = load_mp4()
                content.append(
                    {"video": {"format": "mp4", "source": {"bytes": doc_bytes}}}
                )
...
def scan_for_active_models():
    ...
    try:
        inference_profile_response = bedrock_client.list_inference_profiles().get(
            "inferenceProfileSummaries", []
        )
        inference_profile_summary_models = [
            model["modelArn"]
            for profile in inference_profile_response
    ...
    ddb_config = commons.get_ddb_config(
        config_table, ddb_cache, ddb_cache_timestamp, CACHE_DURATION, logger
    )
    with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
        ...
        for model in active_models:
            ...
            if "TEXT" in output_modalities:
                ...
                if (
                    results[model_id]["IMAGE"] is False
                    and "imported-model" not in model_id
                ):
                    prompts.append(("IMAGE", "what color is this?"))
                else:
                    logger.info("Model %s already has IMAGE capability", model_id)
                if (
                    results[model_id]["VIDEO"] is False
                    and "imported-model" not in model_id
                ):
                ...
                if (
                    results[model_id]["DOCUMENT"] is False
                    and "imported-model" not in model_id
                ):
                ...
                if (
                    results[model_id]["is_kb_model"] is False
                    and "imported-model" not in model_id
                ):
                    if kb_id:
        ...
        for future in concurrent.futures.as_completed(futures):
            model_id, prompt_type, success, input_tokens, output_tokens = (
                future.result()
            )
    ...
    metrics.add_metric(
        name="TotalInputTokens", unit=MetricUnit.Count, value=total_input_tokens
    )
    metrics.add_metric(
        name="TotalOutputTokens", unit=MetricUnit.Count, value=total_output_tokens
    )
...
def test_image_model(model_id) -> bool:
    ...
    if "titan" in model_id or "nova" in model_id:
        image_base64, success_status, error_message = commons.generate_image_titan_nova(
            logger, bedrock_runtime, model_id, "dog", None, None, 5
        )
        if image_base64 is None:
            return False
        return True
    elif "stability" in model_id:
        image_base64, success_status, error_message = (
            commons.generate_image_stable_diffusion(
                logger, bedrock_runtime, model_id, "dog", None, None, None, 5, 10
            )
        )
        if image_base64 is None:
            return False
...
def update_dynamodb(results):
    ...
    try:
        ...
        response = config_table.get_item(
            Key={"user": "models", "config_type": "models"}
        )
        ...
        if "Item" in response:
            ...
            update_expression = (
                "SET config = :new_config, last_update_timestamp = :timestamp"
            )
            expression_attribute_values = {
                ":new_config": results,
                ":timestamp": current_timestamp,
