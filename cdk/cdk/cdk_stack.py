...
class ChatbotWebsiteStack(Stack):
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        if (
            deploy_example_incidents_agent_input is not None
            and deploy_example_incidents_agent_input != ""
        ):
            if (
                "y" in deploy_example_incidents_agent_input.lower()
                or "true" in deploy_example_incidents_agent_input.lower()
            ):
        ...
        lambda_insights_layer_arm64 = _lambda.LayerVersion.from_layer_version_arn(
            self, "LambdaInsightsLayerArm64", lambda_insights_layers_arm64[region]
        )
        ...
        lambda_insights_layer_x86 = _lambda.LayerVersion.from_layer_version_arn(
            self, "LambdaInsightsLayerX86", lambda_insights_layers_x86[region]
        )
        ...
        if pillow_arn:
            pillow_layer = _lambda.LayerVersion.from_layer_version_arn(
                self, "PillowLayer", pillow_arn
            )

        # Create the S3 bucket for website content
        website_content_bucket = s3.Bucket(
            self,
            "GenAiChatbotS3BucketContent",
            removal_policy=RemovalPolicy.DESTROY,
            auto_delete_objects=True,
            enforce_ssl=True,
        )
        cdk_nag.NagSuppressions.add_resource_suppressions(
            website_content_bucket,
            [{"id": "AwsSolutions-S1", "reason": "Server Access Logging Not Required"}],
        )

        # Create the S3 bucket for conversation history
        conversation_history_bucket = s3.Bucket(
            self,
            "ConversationHistoryBucket",
            removal_policy=RemovalPolicy.DESTROY,
            auto_delete_objects=True,
            enforce_ssl=True,
        )
        ...
        schemabucket = s3.Bucket(
            self,
            "GenAiChatbotS3BucketAgentSchemas",
            removal_policy=RemovalPolicy.DESTROY,
            auto_delete_objects=True,
            enforce_ssl=True,
        )
        custom_model_import_bucket = s3.Bucket(
            self,
            "GenAIChatbotS3BucketCustomModelImport",
            removal_policy=RemovalPolicy.DESTROY,
            auto_delete_objects=True,
            enforce_ssl=True,
        )
        ...
        image_origin = origins.S3BucketOrigin.with_origin_access_control(
        ...
        dynamodb_conversations_table = dynamodb.Table(
            self,
            "conversations_table",
            partition_key=dynamodb.Attribute(
        ...
        dynamodb_conversations_table.add_global_secondary_index(
            ...
            sort_key=dynamodb.Attribute(
                name="last_modified_date", type=dynamodb.AttributeType.NUMBER
            ),
            projection_type=dynamodb.ProjectionType.ALL,
        )
        dynamodb_incidents_table_name = "NONE"
        if deploy_example_incidents_agent:
            dynamodb_incidents_table = dynamodb.Table(
                self,
                "incidents_table",
                partition_key=dynamodb.Attribute(
        ...
        dynamodb_configurations_table = dynamodb.Table(
            ...
            sort_key=dynamodb.Attribute(
                name="config_type", type=dynamodb.AttributeType.STRING
            ),
            billing_mode=dynamodb.BillingMode.PAY_PER_REQUEST,
            removal_policy=RemovalPolicy.DESTROY,
        )
        ...
        if allowlist_domain_string is not None and len(allowlist_domain_string) > 0:
            ...
            user_pool.add_trigger(
                cognito.UserPoolOperation.PRE_SIGN_UP, cognito_pre_signup_function
            )
        ...
        image_generation_function.role.add_managed_policy(
        ...
        video_generation_function.role.add_managed_policy(
            iam.ManagedPolicy.from_aws_managed_policy_name("AmazonBedrockFullAccess")
        )
        video_generation_function.role.add_managed_policy(
            iam.ManagedPolicy.from_aws_managed_policy_name(
        ...
        config_function.add_to_role_policy(
            iam.PolicyStatement(
        ...
        agents_client_function = _lambda.Function(
            ...
            code=_lambda.Code.from_asset(
                "./lambda_functions/genai_bedrock_agents_client_fn/"
            ),
            timeout=Duration.seconds(900),
            architecture=_lambda.Architecture.ARM_64,
            tracing=_lambda.Tracing.ACTIVE,
        ...
        agents_client_function.role.add_managed_policy(
            iam.ManagedPolicy.from_aws_managed_policy_name("AmazonBedrockFullAccess")
        )
        agents_client_function.role.add_managed_policy(
            iam.ManagedPolicy.from_aws_managed_policy_name(
        ...
        if deploy_example_incidents_agent:
            incidents_agents_function = _lambda.Function(
                ...
                code=_lambda.Code.from_asset(
                    "./lambda_functions/genai_bedrock_incidents_agents_fn/"
                ),
        ...
        lambda_async_function.role.add_managed_policy(
            iam.ManagedPolicy.from_aws_managed_policy_name("AmazonBedrockFullAccess")
        )
        lambda_async_function.role.add_managed_policy(
            iam.ManagedPolicy.from_aws_managed_policy_name(
        ...
        model_scan_function = _lambda.Function(
            ...
            code=_lambda.Code.from_asset(
                "lambda_functions/genai_bedrock_model_scan_fn/"
            ),
            timeout=Duration.seconds(900),
            architecture=_lambda.Architecture.ARM_64,
            tracing=_lambda.Tracing.ACTIVE,
        ...
        model_scan_function.role.add_managed_policy(
            iam.ManagedPolicy.from_aws_managed_policy_name("AmazonBedrockFullAccess")
        )
        model_scan_function.role.add_managed_policy(
            iam.ManagedPolicy.from_aws_managed_policy_name(
        ...
        # Add permissions to the Lambda function's role
        model_scan_function.add_to_role_policy(
            iam.PolicyStatement(
        ...
        presigned_url_function = _lambda.Function(
            ...
            code=_lambda.Code.from_asset(
                "./lambda_functions/genai_bedrock_presigned_fn"
            ),
            timeout=Duration.seconds(20),
            architecture=_lambda.Architecture.ARM_64,
            tracing=_lambda.Tracing.ACTIVE,
        ...
        presigned_url_function.add_to_role_policy(
            iam.PolicyStatement(
        ...
        send_message_queue = sqs.Queue(
            self, "SendMessageQueue", visibility_timeout=Duration.seconds(900)
        )
        presigned_url_queue = sqs.Queue(
            self, "PresignedUrlQueue", visibility_timeout=Duration.seconds(60)
        )
        model_scan_request_queue = sqs.Queue(
            self, "ModelScanRequestQueue", visibility_timeout=Duration.seconds(900)
        )

        # Create IAM Roles for API Gateway to send messages to SQS
        send_message_role = iam.Role(
            self,
            "SendMessageApiGatewayRole",
            assumed_by=iam.ServicePrincipal("apigateway.amazonaws.com"),
        )
        send_message_role.add_to_policy(
            iam.PolicyStatement(
                actions=["sqs:SendMessage"], resources=[send_message_queue.queue_arn]
            )
        )

        presigned_url_role = iam.Role(
            self,
            "PresignedUrlApiGatewayRole",
            assumed_by=iam.ServicePrincipal("apigateway.amazonaws.com"),
        )
        presigned_url_role.add_to_policy(
            iam.PolicyStatement(
                actions=["sqs:SendMessage"], resources=[presigned_url_queue.queue_arn]
            )
        )

        model_scan_request_role = iam.Role(
            self,
            "ModelScanRequestApiGatewayRole",
            assumed_by=iam.ServicePrincipal("apigateway.amazonaws.com"),
        )
        model_scan_request_role.add_to_policy(
            iam.PolicyStatement(
        ...
        send_message_resource.add_method(
            "POST",
                ...
                options=apigw.IntegrationOptions(
                    ...
                    integration_responses=[
                        apigw.IntegrationResponse(
                            status_code="200",
                            response_templates={"application/json": '{"done": true}'},
                        )
                    ],
                ),
            ),
        ...
        model_scan_request_resource.add_method(
            "POST",
            apigw.AwsIntegration(
                service="sqs",
        ...
        lambda_router_function.add_event_source(
            lambda_event_sources.SqsEventSource(send_message_queue)
        )

        model_scan_function.add_event_source(
            lambda_event_sources.SqsEventSource(model_scan_request_queue)
        )
        ...
        websocket_api.add_route(
            "config", integration=config_fn_integration, return_response=True
        )

        websocket_api.add_route(
            "$default", integration=bedrock_fn_integration, return_response=True
        )
        ...
        config_function.add_environment(
            "SCHEDULE_NAME", module_scan_schedule.schedule_name
        )
                ...
                "cognito-identity.amazonaws.com",
                {
                    "StringEquals": {
                        "cognito-identity.amazonaws.com:aud": (
                            self.region + ":abcdefg-1234-5678-910a-0e8443553f95"
                        )  # an impossible ID
                    },
        ...
        app_monitor_arn = (
            f"arn:aws:rum:{region}:{self.account}:appmonitor/{cfn_app_monitor.attr_id}"
        )
        ...
        CfnOutput(
            self,
            "AWSChatBotURL",
            value=f"https://{cloudfront_distribution_domain_name}",
        )
